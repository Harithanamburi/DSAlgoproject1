	...
	private By arun = By.xpath("//button[@type='button']");
		...
		public void linkedListValiddata() throws InterruptedException {
			...
			for (int i = 0; i < linkedlistLinks.size(); i++) {
				...
				List<WebElement> leftPanelSublinks1 = driver.findElements(leftPanelSublinks);
				String originalStyle = leftPanelSublinks1.get(i).getAttribute("style");
				JavascriptExecutor js = (JavascriptExecutor) driver;
				js.executeScript("arguments[0].setAttribute(arguments[1], arguments[2])", leftPanelSublinks1.get(i), "style",
		...
		public void linkedListInValiddata() throws InterruptedException {
			...
			for (int i = 0; i < linkedlistLinks.size(); i++) {
				...
				List<WebElement> leftPanelSublinks2 = driver.findElements(leftPanelSublinks);
				String originalStyle = leftPanelSublinks2.get(i).getAttribute("style");
				JavascriptExecutor js = (JavascriptExecutor) driver;
				js.executeScript("arguments[0].setAttribute(arguments[1], arguments[2])", leftPanelSublinks2.get(i), "style",
						originalStyle + "border: 2px solid red;");
